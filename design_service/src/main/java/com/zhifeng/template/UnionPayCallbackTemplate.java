package com.zhifeng.template;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Component;

import java.util.HashMap;
import java.util.Map;

/**
 * 模拟银联
 *
 * @author ganzhifeng
 * @className UnionPayCallbackTemplate
 * @date 2019/9/17 0017
 */
@Component
public class UnionPayCallbackTemplate extends AbstractPayCallbackTemplate {

    private static final Logger logger = LoggerFactory.getLogger(UnionPayCallbackTemplate.class);

    /**
     * 假设银联报文
     *
     * @return
     */
    @Override
    protected Map<String, Object> verifySignTrue() {

        logger.info("解析银联报文");
        Map<String, Object> verifySignature = new HashMap<>();
        verifySignature.put("price", 1399);
        verifySignature.put("orderDes", "购买成功");
        // 支付状态为1表示为成功
        verifySignature.put("paymentStatus", 1);
        verifySignature.put("orderNumber", 201909170018L);
        // 解析报文是否成功 0 为成功
        verifySignature.put("analysisCode", 0);
        return verifySignature;
    }

    @Override
    protected String asyncService(Map<String, Object> verifySignatureMap) {
        logger.info("业务处理");
        Integer paymentStatus = (Integer) verifySignatureMap.get("paymentStatus");
        if (paymentStatus.intValue() == 1) {
            Long orderNumber = (Long) verifySignatureMap.get("orderNumber");
            logger.info("订单【" + orderNumber + "】,已经支付成功 修改订单状态为已经支付...");
            return resultSuccess();
        }
        return resultFail();
    }

    @Override
    protected String resultSuccess() {
        return String.valueOf(Boolean.TRUE);
    }

    @Override
    protected String resultFail() {
        return String.valueOf(Boolean.FALSE);
    }
}
